...
import { faMagnifyingGlass, faSpinner, faFaceFrown } from "@fortawesome/free-solid-svg-icons";
...
// SEARCH FUNCTION
      ...
      .then((data) => {
        ...
        if (data.Response === "True") {
          ...
          const fetchPromises = films.map((movie) =>
            fetch(`https://www.omdbapi.com/?apikey=e1f6fd51&i=${movie.imdbID}`).then((res) => res.json())
          );
  ...
  // NOTHING FOUND
  const notFoundEl = <div className="not-found">
    Sorry, couldn't find that movie<FontAwesomeIcon icon={faFaceFrown} className="not-found-icon" />
    </div>;
